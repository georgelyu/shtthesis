...
\subsection{累积量碰撞模型}
\label{sec:cumulant}
下面我们介绍累积量碰撞模型。累积量碰撞模型相比其它模型 (原始矩、中心矩等) 有着遵守伽利略不变性、各个自由度之间的耦合程度低、超黏度 (hyper-viscosity) 误差更小的优势。在这里我们先简要地介绍累积量碰撞模型本身的计算过程。

不同的碰撞模型的本质是在探索什么量可以最优地描述分布函数，我们称这些量为可观察量 (observable variables)。在累积量方法中，不同累积量之间是统计独立的 (statistically independent)，也就是说，两个累积量的联合概率等于各自概率的积。这样可以使得低阶累积量的松弛不影响高阶累积量，而中心矩无法做到高阶矩和低阶矩的完全解耦，从而精度不如累积量模型。并且在中心矩模型中，由于高阶矩依赖于低阶矩，高阶矩的松弛会影响本应由黏度控制的低阶矩的松弛，造成超黏度误差~\citep{Geier-2015}。为了推导出累积量，我们先将分布函数$f$转换到频率空间$F$中，这样便于我们进行泰勒展开。到频域的变换由拉普拉斯变换 (Laplace transform) 完成：
\begin{equation}
...
\subsection{插值反弹边界}
简单反弹边界的一个问题是只有边界在速度方向中点时，边界处理的精度才可达到二阶。边界处于其他位置时，边界处理的精度只有一阶。我们这里说的阶数指的是收敛速度，即对于准确解$u$，我们的数值解$u_h$可以满足误差$E(h)=\| u-u_h \|=O(\delta x)^n$，则该解的精度为$n$阶。
