...
\end{equation}
然而，简单反弹边界的简洁也带来了一些局限性。因为分布函数永远假设在速度方向的中间被反弹，这会产生较大的速度梯度与色散误差，并给高雷诺数湍流仿真带来很强的不稳定性。这一点在第~\ref{chap:siga21} 章中有更详细的说明。
...
SI-IBM的一个优势是通过插值，固体边界的大速度梯度会被压制，从而提升稳定性。但是由于它需求有固体内部点参与计算，对于亚网格尺度的物体，SI-IBM并不适合。这一点同样在第~\ref{chap:siga21} 章中有更详细的说明。同时，在物体运动时，SI-IBM需要格点重填 (node refilling)，这会引入速度外插之外的新的误差。
